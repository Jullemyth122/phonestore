...
router.delete('/cart/:email/remove-checked-items', async (req, res) => {
    try {
        ...
        const uniqueBrandModelPairs = [...new Set(removedItems.map(item => `${item.brand}-${item.model}`))];

        // Update or create orders in the Orders schema
        for (const pair of uniqueBrandModelPairs) {
            const [brand, model] = pair.split('-');
            const sellerCount = removedItems.filter(item => item.brand === brand && item.model === model).length;

            // Find an existing order with the same brand and model
            const existingOrder = await Order.findOne({ brand, model });
            ...
            if (existingOrder) {
                ...
                await Order.create({
                    ...
                    model,
                    sellerCount,
                    name: removedItems.find(item => item.brand === brand && item.model === model).name,
